<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-text>
            ...
            <Number label="Tel" v-model="value.tel" :editMode="editMode"/>
    ...
    export default {
        ...
        methods: {
            ...
            async pickCook() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['pickcook'].href))
            ...
            async acceptDelivery() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['acceptdelivery'].href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
